													...
													<tbody>
														...
														<c:forEach var="it" items="${students}" varStatus="i">
															<tr>
																...
																<c:forEach var="curr" items="${curriculums}"
																	varStatus="j">
																	<c:set var="stucodeexpid"
																		value="${it.id}${curr.experiment_id}"></c:set>
																	<td>
																		<c:if test="${smap[stucodeexpid]==-99}">
																			<a target="_blank"
																				href="${basePath}superadmin/experimentrecord?operate=edit&id=${eridmap[stucodeexpid]}">
																				<img src="${basePath}bssets/i/greendot.png" />
																			</a>
																		</c:if> <c:if test="${smap[stucodeexpid]==null}">
																			<img src="${basePath}bssets/i/reddot.png" />
																		</c:if>
																		<c:if
																			test="${smap[stucodeexpid]!=null && smap[stucodeexpid]!=-99}">
																			<a target="_blank"
																				href="${basePath}superadmin/experimentrecord?operate=edit&id=${eridmap[stucodeexpid]}">${smap[stucodeexpid]}</a>
																		</c:if>
																	</td>
																</c:forEach>
															</tr>
