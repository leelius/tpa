			...
			<div class="row">
				<div class="col-md-12">
					...
					<c:if test="${exprecord != null}">
						<%-- julius	--%>
						<div class="text-success">你已经完成该实验${exprecord.scorescript}</div>
					</c:if>
				</div>
				<hr/>
				<div class="col-md-12">
					<!-- 表单开始 -->
					<form action="${basePath}student/expresult" id="form1" method="post" enctype="multipart/form-data">
						<input type="hidden" name="lecid" value="${lec.id}" />
						<input type="hidden" name="expid" value="${exp.id}" />
						<input type="hidden" id="operate" name="operate" value="clicksubmit" />
						<input type="hidden" id="changedflag" name="changedflag" value="nochanged" />
						<input type="hidden" id="mainflow" name="mainflow" />
						...
						<div id="itemContent">
							<div class="itemIntrotitle">
								...
								<c:if test="${closetime>now}">
									<textarea class="form-control" id="mainflowet" name="mainflowet"
									...
									--%>
											<li class="am-margin-vertical">
												<span class="am-text-danger">
													第${xh.count}个文件：${it.filename};
													&nbsp;&nbsp;&nbsp;&nbsp;文件大小：${it.filesize}字节
												</span>
												&nbsp;&nbsp;&nbsp;&nbsp;
												<a href="student/expshow/${lec.id}?operate=deletefile&fileid=${it.filehash}"
						...
						<c:if test="${closetime>now}">
							<br/>
							<input type="button" value="保存" id="submitbtn" class="btn btn-secondary" />
						</c:if>
					</form>
					<!-- 表单结束 -->
				</div>
	...
	<c:if test="${closetime>now}">
		<script type="text/javascript">
			...
			KindEditor.ready(function(K) {
						editor1 = K.create('textarea[name="mainflowet"]', {
						...
						editor2 = K.create('textarea[name="resultet"]', {
											cssPath : '${basePath}bssets/ket/plugins/code/prettify.css',
											uploadJson : '${basePath}admin/uploadfile',
											fileManagerJson : '${basePath}admin/filemanager',
											allowFileManager : true,
