														...
														<c:forEach var="it" items="${list}" varStatus="index">
															...
															<tr>
																<td colspan="6">
																	<div>
																		实验详情
																		<a href="${pagename}?operate=additem&id=${it.curriculum.id}&sid=${it.curriculum.subject_id}" class="btn btn-info">
																			增加一个实验
																		</a>
																	</div>
																	<table class="table table-striped table-bordered table-hover">
																		...
																		<c:forEach var="exp" items="${it.experimentlist}" varStatus="j">
																			<tr>
																				<td>${j.count}</td>
																				<td>${exp.experiment_title}</td>
																				<td>
																					<c:if test="${exp.opening_gmt !=0}">
																						<fmt:formatDate pattern="yyyy-MM-dd" value="${exp.opening_gmt_date}" />
																					</c:if>~<c:if test="${exp.closing_gmt !=0}">
																						<fmt:formatDate pattern="yyyy-MM-dd" value="${exp.closing_gmt_date}" />
																					</c:if>
																					<c:choose>
																				...
																				<td>
																					<c:choose>
																						<c:when test="${exp.experiment_curriculum_is_actived}">
																							<a href="${pagename}?operate=hideitem&id=${exp.id}" class="btn btn-warning">关闭</a>
																						</c:when>
																						<c:when test="${!exp.experiment_curriculum_is_actived}">
																							<a href="${pagename}?operate=showitem&id=${exp.id}" class="btn btn-info">开通</a>
																						</c:when>
																					</c:choose>
																					<a href="${pagename}?operate=edititem&id=${exp.id}" class="btn btn-success">编辑</a>
																				</td>
																			</tr>
																		</c:forEach>
																	</table>
					...
					var editor1 = K
							.create(
									'textarea[name="content"]',
									{
										...
										uploadJson : '${basePath}admin/uploadfile?category=article',
