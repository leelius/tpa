...
public class SuperAdminController {
	...
	private static final Log logger = LogFactory.getLog(SuperAdminController.class);
	...
	public SuperAdminController() {
		System.out.println("SuperAdminController--控制器加载");
	}
	...
	public void modelattribute(Model model, HttpSession session) {
		...
		// System.out.println("Admin.modelattribute(添加共有属性)");
	...
	public ModelAndView index(ModelAndView mv, HttpSession session) {
		...
		System.out.println("superadmin/index had been called!");
		...
		mv.setViewName("superadmin/index");
		return mv;
	}
	...
	public @ResponseBody Map<String, Object> ajaxformupload(HttpServletRequest request, HttpSession session,
			@RequestParam("uploadfile") MultipartFile uploadfile) throws IOException {

		System.out.println("Admin.ajaxformupload()进入了文件上传部分");
	...
	public @ResponseBody Map<String, Object> permitCheckIsExist(HttpServletRequest request,
			HttpServletResponse response) throws IOException {

		String code = request.getParameter("code");
		String operate = request.getParameter("operate");
		String id = request.getParameter("id");
		// System.out.println("Admin.managePermitCheckIsExist()code=" + code +
		...
		if (operate.equals("edit") && !StringUtils.isBlank(id)) {
			for (int i = 0; i < list.size(); i++) {
				// System.out.println("Admin.managePermitCheckIsExist()id=" + id + ",getid=" +
				// list.get(i).getId());
		...
		// System.out.println("Admin.managePermitCheckIsExist()list.size()=" +
		// list.size());
		if (list.size() == 0) {
		...
		// System.out.println("Admin.managePermitCheckIsExist()map=" + map);
	...
	public ModelAndView permitGET(ModelAndView mv, @RequestParam(value = "kw", required = false) String kw,
		...
		String pagename = "superadmin/permit";
		...
		if (!StringUtils.isBlank(operate)) {
			...
			ordlist.add(new KeyAndValue(Permit.ID, "asc"));
	...
	public ModelAndView userpasswordGET(ModelAndView mv, @RequestParam(value = "kw", required = false) String kw,
		...
		String pagename = "superadmin/userpassword";
		mv.addObject("tagpostfixlist", Common.tagPostfix);
	...
	public ModelAndView roleGET(ModelAndView mv, @RequestParam(value = "kw", required = false) String kw,
		...
		System.out.println(this.getClass().getName() + ",operate=" + operate + ",id=" + id + ",pn=" + pn + ",ps=" + ps
				+ ",kw=" + kw + ",clrkw=" + clrkw);
		mv.addObject("navitem", "User");// 菜单项显示当前项
		String pagename = "superadmin/role";
		...
		if (!StringUtils.isBlank(operate)) {
			...
			List<KeyAndValue> ordlist = new ArrayList<KeyAndValue>();
			ordlist.add(new KeyAndValue(Role.ID, "asc"));
	...
	public ModelAndView rolePOST(ModelAndView mv, HttpServletRequest request, HttpSession session) throws Exception {
		...
		case "addform": {
			...
			if (StringUtils.isBlank("name") || StringUtils.isBlank("alias")) {
				...
				for (int i = 0; i < permit_ids.size(); i++) {
					// System.out.println("Admin.userPOST()role_ids=" + role_ids.get(i));
		...
		case "editform": {
			...
			if (list.size() > 1 || (list.size() == 1 && list.get(0).getId() != id)) {
				...
				// for (int i = 0; i < permit_ids.size(); i++) {
				// System.out.println("Admin.userPOST()role_ids=" + permit_ids.get(i));
				// }
				...
				for (int i = 0; i < permitoldlist.size(); i++) {
					if (!permit_ids.contains(permitoldlist.get(i))) {
						// 已经删除了的权限
						System.out.println("Admin.userPOST()需要删除：" + permitoldlist.get(i));
				...
				for (int i = 0; i < permit_ids.size(); i++) {
					if (!permitoldlist.contains(permit_ids.get(i))) {
						// 应该增加的角色
						System.out.println("Admin.userPOST()需要增加：" + permit_ids.get(i));
	...
	public ModelAndView userGET(ModelAndView mv, @RequestParam(value = "kw", required = false) String kw,
		...
		System.out.println(this.getClass().getName() + ",operate=" + operate + ",id=" + id + ",pn=" + pn + ",ps=" + ps
				+ ",kw=" + kw + ",clrkw=" + clrkw);
		mv.addObject("navitem", "User");// 菜单项显示当前项
		String pagename = "superadmin/user";
		// 新增和编辑，列表页面筛选也需要使用，页面部分都有一些预先读取的内容
		if (StringUtils.isBlank(operate) || operate.equals("add") || operate.equals("edit")) {
			// 新建、编辑、列表的条件筛选
			// 用于制作彩色的标签
...
//				}
				...
				for (int i = 0; i < roleoldlist.size(); i++) {
					if (!roleoldlist.get(i).equals("superadmin") && !role_ids.contains(roleoldlist.get(i))) {
						// 已经删除了的角色
						// System.out.println("Admin.userPOST()需要删除：" + roleoldlist.get(i));
						// 找到对应的roldid
						Integer roleid = 0;
				...
				for (int i = 0; i < role_ids.size(); i++) {
					if (!roleoldlist.contains(role_ids.get(i))) {
						// 应该增加的角色
						// System.out.println("Admin.userPOST()需要增加：" + role_ids.get(i));
						// 找到对应的roldid
						Integer roleid = 0;
	...
	public ModelAndView userinfoGET(ModelAndView mv, @RequestParam(value = "kw", required = false) String kw,
		...
		System.out.println(
				this.getClass().getName() + ",operate=" + operate + ",id=" + id + ",kw=" + kw + ",clrkw=" + clrkw);
		mv.addObject("navitem", "Personal");// 菜单项显示当前项
		String pagename = "superadmin/userinfo";
		mv.addObject("tagpostfixlist", Common.tagPostfix);
		mv.addObject("operate", "edit");// 触发下拉的脚本和样式
	...
	public ModelAndView syslogGET(ModelAndView mv, @RequestParam(value = "kw", required = false) String kw,
		...
		String pagename = "superadmin/syslog";
		// 新增和编辑，列表页面筛选也需要使用，页面部分都有一些预先读取的内容
		if (StringUtils.isBlank(operate) || operate.equals("add") || operate.equals("edit")) {
			// 新建、编辑、列表的条件筛选
			// 用于制作彩色的标签
	...
	public ModelAndView subjectGET(ModelAndView mv, @RequestParam(value = "kw", required = false) String kw,
		...
		String pagename = "superadmin/subject";
		// 新增和编辑，列表页面筛选也需要使用，页面部分都有一些预先读取的内容
		if (StringUtils.isBlank(operate) || operate.equals("add") || operate.equals("edit")) {
			// 新建、编辑、列表的条件筛选
			// 用于制作彩色的标签
	...
	public ModelAndView curriculumGET(ModelAndView mv, @RequestParam(value = "kw", required = false) String kw,
		...
		System.out.println(this.getClass().getName() + ",operate=" + operate + ",id=" + id + ",pn=" + pn + ",ps=" + ps
				+ ",kw=" + kw + ",clrkw=" + clrkw);
		mv.addObject("navitem", "BaseInfo");// 菜单项显示当前项
		String pagename = "superadmin/curriculum";
		// 新增和编辑，列表页面筛选也需要使用，页面部分都有一些预先读取的内容
		if (StringUtils.isBlank(operate) || operate.equals("add") || operate.equals("edit")) {
			// 新建、编辑、列表的条件筛选
			// 用于制作彩色的标签
	...
	public ModelAndView experimentmanagementGET(ModelAndView mv,
		...
		// 管理员和教师共用一个页面
		String pagename = "superadmin/experimentmanagement";
		...
		if (!StringUtils.isBlank(operate)) {
			...
			for (int i = 0; i < curriculums.size(); i++) {
				...
				// 老师的实验
				List<Vlink_experiment_curriculum_min> experiments = svc
						.listExperimentByMap(new HashMap<String, Object>() {
							{
			...
			mv.addObject("list", list);
			// System.out.println("Admin.vcurriculumGET()list.size=" + list.size());
	 ...
	 */
	@RequestMapping(value = { "/experimentmanagement", "/experimentmanagement/",
			"/experimentmanagement/index.html" }, method = RequestMethod.POST)
	public ModelAndView experimentmanagementPOST(ModelAndView mv, HttpServletRequest request, HttpSession session)
			throws Exception {

		System.out.println(this.getClass().getName() + "POST");
		String pagename = "superadmin/experimentarrange";
		UserSession userSession = (UserSession) (session.getAttribute(Common.sessionUser));
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		String operate = request.getParameter("operate");
		// --------编辑修改共有的部分---------
...
//		Boolean is_actived = StringUtils.isBlank(request.getParameter("is_actived")) ? false : true;
	...
	public ModelAndView statexperimentGET(ModelAndView mv, HttpServletRequest request, HttpSession session) {
		...
		UserSession userSession = (UserSession) (session.getAttribute(Common.sessionUser));
		// System.out.println("Admin.experimentcurriculumGET()userSession="+userSession.toString());

		mv.addObject("navitem", "Experiment");// 菜单项显示当前项
		String pagename = "superadmin/statexperiment";
		...
		System.out.println("Admin.statexperimentGET()semester=" + semester);
		List<StatExperimentCurriculum> list = svc.statExperimentCurriculum(semester);
		// System.out.println("Admin.vlink_experiment_curriculum_minGET()list.size=" +
		// list.size());
	...
	public ModelAndView statexperimentteacherGET(ModelAndView mv, HttpServletRequest request, HttpSession session) {
		...
		mv.addObject("navitem", "Experiment");// 菜单项显示当前项
		String pagename = "superadmin/statexperimentteacher";
		...
		System.out.println("Admin.statexperimentteacherGET()userSession.getAlias()=" + userSession.getAlias());
	...
	public ModelAndView evaluationGET(ModelAndView mv, @RequestParam(value = "kw", required = false) String kw,
		...
		String pagename = "superadmin/evaluation";
		// 新增和编辑，列表页面筛选也需要使用，页面部分都有一些预先读取的内容
		if (StringUtils.isBlank(operate) || operate.equals("add") || operate.equals("edit")) {
			// 新建、编辑、列表的条件筛选
			// 用于制作彩色的标签
	...
	public ModelAndView evaluationPOST(ModelAndView mv, HttpServletRequest request, HttpSession session)
		...
		System.out.println(this.getClass().getName() + "POST");
		String pagename = "superadmin/vevaluation";
		UserSession userSession = (UserSession) (session.getAttribute(Common.sessionUser));
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		String operate = request.getParameter("operate");
		// --------编辑修改共有的部分---------
	...
	public ModelAndView configEvaluationGET(ModelAndView mv, @RequestParam(value = "id", required = false) String id,
			...
			HttpSession session) {
		System.out.println("Admin.configEvaluationGET()id=" + id + ",operate=" + operate);

		mv.addObject("navitem", "Evaluation");// 菜单项显示当前项
		String pagename = "superadmin/configevaluation";
		...
		if (!StringUtils.isBlank(operate)) {
			...
			switch (operate) {
			case "cena": {
				// 先用离线的cena评测，cena会在学生提交的目录先生成一个result.xml，这个文件中包括了所有题目评测的信息，包括成绩
				Integer evaluation_id = Integer.parseInt(id);
				System.out.println("SuperAdminController.configEvaluationGET()evaluation_id=" + evaluation_id);

				// 找到学生的提交记录
				List<Evaluation_result> erlist = svcb.listEvaluation_resultByMap(new HashMap<String, Object>() {
					{
						put(Evaluation_result.EVALUATION_ID, evaluation_id);
					}
				});
				// erlist.size()
				for (int i = 0; i < erlist.size(); i++) {

					String sno = erlist.get(i).getSno();
					String xmlfile = "";
					Integer totalscore = 0;
					// 找学生的提交每个小题内容
					List<Evaluation_result_record> errlist = svcb
							.listEvaluation_result_recordByMap(new HashMap<String, Object>() {
								{
									put(Evaluation_result_record.EVALUATION_ID, evaluation_id);
									put(Evaluation_result_record.SNO, sno);

								}
							});
					if (errlist.size() > 0) {
						// TODO: 查找文件的目录
						String uploadfolder = "WEB-INF\\evfiles\\ug" + errlist.get(0).getStudent_group_id() + "e"
								+ evaluation_id + "\\src\\s" + sno;
						String savefolder = request.getServletContext().getRealPath(uploadfolder);

						xmlfile = savefolder + "\\result.xml";

						System.out.println("SuperAdminController.configEvaluationGET()savefolder=" + savefolder);
					}
					...
					if (xml.exists()) {
						...
						for (Element element : elementList) {
							...
							Iterator problem = element.elementIterator();
							while (problem.hasNext()) {
								Element p = (Element) problem.next();
								// System.out.println("节点名：" + p.getName() + "---节点值：" + p.getStringValue());
								Iterator testcase = p.elementIterator();
								while (testcase.hasNext()) {

									Element t = (Element) testcase.next();
									Integer score = Integer.parseInt(t.attribute("score").getValue());
									totalscore += score;
						...
						// System.out.println("有cena评测结果");
						System.out.println(
								"SuperAdminController.configEvaluationGET()" + ",evaluation_id=" + evaluation_id
										+ ",sno=" + sno + ",totalscore=" + totalscore + ",id=" + erlist.get(i).getId());

						boolean rtv = svc.updateEvaluation_resultById(evaluation_id, sno, totalscore,
								erlist.get(i).getId());
						System.out.println("SuperAdminController.configEvaluationGET()rtv=" + rtv);
				...
				mv.addObject("jumpto", Common.getBasePath() + pagename + "?id=" + evaluation_id);
				mv.setViewName("jumperclick");
			}
				break;
			case "builditem": {
				System.out.println("Admin.configEvaluationGET()重置一个考生的考核内容,uid=" + uid + ",id=" + id);
			...
			case "builditems": {
				...
				break;
			case "viewitem": {
				System.out.println("Admin.managevevaluationGET()查看一个考生的考核详情,uid=" + uid + ",id=" + id);
			...
			// 读取当前考核信息
			// System.out.println("Admin.managevevaluationGET()itemid=" + itemid);
			Vevaluation evaluation = svcb.getVevaluationById(itemid);
			mv.addObject("evaluation", evaluation);
			// 读取考核对应的分组所有成员
...
//			group by subject_id,student_id
			...
			HashMap<Integer, Integer> evScoreMap = new HashMap<>();
			{
				...
				System.out.println("Admin.managevevaluationGET()statlist.size(已经提交考核人数)=" + statlist.size());
				for (int i = 0; i < statlist.size(); i++) {

					// System.out.println("Admin.configEvaluationGET()"+statlist.get(i).toString());
...
//				HashMap<String, Integer> evScoreMap = svcEvpr.getEvScoreMap();
			...
			for (int i = 0; i < vlink_student2group.size(); i++) {
				...
				// System.out.println("Admin.configEvaluationGET()it=" + it.toString());
				list.add(it);
			}
	...
	public ModelAndView evaluationReviewGET(ModelAndView mv,
		...
		System.out.println("Admin.evaluationReviewGET()evid=" + evid + ",studentid=" + studentid);

		mv.addObject("navitem", "Evaluation");// 菜单项显示当前项
		String pagename = "superadmin/evaluationreview";
		...
		for (int i = 0; i < evcontentlist.length; i++) {
			...
			for (int j = 0; j < evprlist.size(); j++) {
				...
				String savefolder = request.getServletContext().getRealPath(uploadfolder);
				// System.out.println("Admin.evaluationReviewGET(查找当前考题，已经上传的文件)savefolder=" +
				// savefolder);
		...
		System.out.println("Admin.evaluationReviewGET()scoretotal=" + scoretotal);
		return mv;
	}
	...
	public ModelAndView statevaluation_pointGET(ModelAndView mv,
		...
		System.out.println(this.getClass().getName() + ",operate=" + operate + ",id=" + id + ",pn=" + pn + ",ps=" + ps
				+ ",kw=" + kw + ",clrkw=" + clrkw);
		mv.addObject("navitem", "Evaluation");// 菜单项显示当前项
		String pagename = "superadmin/statevaluationpoint";
	...
	public ModelAndView evaluation_pointGET(ModelAndView mv, @RequestParam(value = "kw", required = false) String kw,
		...
		mv.addObject("navitem", "Evaluation");// 菜单项显示当前项
		String pagename = "superadmin/evaluationpoint";
		...
		if (!StringUtils.isBlank(operate)) {
			...
			case "add": {
				...
				System.out.println("Admin.evaluation_pointGET()sid=" + subject_id + ",type=" + type);

				mv.setViewName("superadmin/evaluationpoint_add");
			}
				break;
			case "edit": {
				...
				case 6: {
					...
					break;
				case 7: {
					PevProgramPoint pevpoint = new PevProgramPoint(it, evtMap);
					mv.addObject("item", pevpoint);
					mv.addObject("type", svcb.getEvaluation_typeById(pevpoint.getType()));
				}
					break;
				...
				// mv.setViewName(pagename);
				mv.setViewName("superadmin/evaluationpoint_edit");
	...
	public ModelAndView evaluation_pointPOST(ModelAndView mv, HttpServletRequest request, HttpSession session)
		...
		case "editform": {
			...
			case 7: {
				...
				String title = request.getParameter("title");
				// 输入
				String evpin = request.getParameter("evpin");
				// 输出
				String evpout = request.getParameter("evpout");
				// 样例输入
				String evpexamplein = request.getParameter("evpexamplein");
				// 样例输出
				String evpexampleout = request.getParameter("evpexampleout");
				// 数据限制
				String datalimit = request.getParameter("datalimit");

				PevProgramPoint pgpoint = new PevProgramPoint(id, subject_id, true, type, difficulty, evptitle, title,
						evpin, evpout, evpexamplein, evpexampleout, datalimit);
				String content = pgpoint.toXMLString();
				// ----------保存前检查逻辑上是否有冲突---------------
				svcb.updateEvaluation_pointById(subject_id, true, type, difficulty, content, new Date().getTime(), id);
				// ----------保存前检查逻辑问题结束------------
			}
		...
		mv.addObject("jumpto", Common.getBasePath() + "superadmin/evaluationpoint?sid=" + subject_id + "&type=" + type);
		mv.setViewName("jumper");
	...
	public ModelAndView student_groupGET(ModelAndView mv, @RequestParam(value = "kw", required = false) String kw,
		...
		System.out.println(this.getClass().getName() + ",operate=" + operate + ",id=" + id + ",pn=" + pn + ",ps=" + ps
				+ ",kw=" + kw + ",clrkw=" + clrkw);
		mv.addObject("navitem", "BaseInfo");// 菜单项显示当前项
		String pagename = "superadmin/studentgroup";
		// 新增和编辑，列表页面筛选也需要使用，页面部分都有一些预先读取的内容
		if (StringUtils.isBlank(operate) || operate.equals("add") || operate.equals("edit")) {
			// 新建、编辑、列表的条件筛选
			// 用于制作彩色的标签
	...
	public ModelAndView student_groupPOST(ModelAndView mv, HttpServletRequest request, HttpSession session)
		...
		System.out.println(this.getClass().getName() + "POST");
		String pagename = "superadmin/student_group";
		UserSession userSession = (UserSession) (session.getAttribute(Common.sessionUser));
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		String operate = request.getParameter("operate");
		// --------编辑修改共有的部分---------
	...
	public ModelAndView vlink_student2groupGET(ModelAndView mv, @RequestParam(value = "kw", required = false) String kw,
		...
		System.out.println(this.getClass().getName() + ",operate=" + operate + ",id=" + id + ",pn=" + pn + ",ps=" + ps
				+ ",kw=" + kw + ",clrkw=" + clrkw);
		mv.addObject("navitem", "BaseInfo");// 菜单项显示当前项
		String pagename = "superadmin/student";
		// 新增和编辑，列表页面筛选也需要使用，页面部分都有一些预先读取的内容
		if (StringUtils.isBlank(operate) || operate.equals("add") || operate.equals("edit")) {
			// 新建、编辑、列表的条件筛选
			// 用于制作彩色的标签
...
//			 mv.addObject("student_grouplist", svcb.listStudent_groupOrderByAsc(Student_group.ID));
		...
		if (!StringUtils.isBlank(operate)) {
			...
			switch (operate) {
			case "add": {
				mv.addObject("pagetitle", "学生管理");
				mv.addObject("subtitle", "Student management");
				mv.addObject("formtitle", "新增学生");
				mv.addObject("pagename", pagename);
				mv.addObject("param", svcb.mapStudent_group());
				// 默认作者是当前用户
				UserSession userSession = (UserSession) (session.getAttribute(Common.sessionUser));
				mv.addObject("author", userSession.getLogin_name());
				// 默认置顶3天
	...
	/**
	 * 导入从教务系统下载的学生账号
	 * 通过excel表，向指定班级，批量导入学生信息 -julius
	 * @param mv
	 * @param request
	 * @param session
	 * @param uploadfile
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = { "/studentimport", "/studentimport/", "/studentimport/index.html" }, method = RequestMethod.POST)
	public ModelAndView vlink_student2groupPOSTImportFormXls(ModelAndView mv, HttpServletRequest request,
		...
		if (uploadfile.isEmpty()) {
			...
			mv.addObject("jumpto", Common.getBasePath() + "superadmin/student?operate=add&sgid=" + student_group_id);
			mv.setViewName("jumperback");
			return mv;
		} else {
			...
			for (int i = 1; i < rows; i++) {
				...
				Student it = null;
				String login_name = rs.getCell(7, i).getContents();// 学号 第8列都是学号 第2行开始
				if (StringUtils.isBlank(login_name)) {
					System.out.println("Admin.usergroupPOST()出现空白学号，自动跳过" + login_name);
					// 学号信息有误！
					numberError++;
				} else {
					// ----------保存前检查逻辑问题---------------
					...
					if (list.size() == 0) {
						...
						System.out.println("Admin.managevuserPOST()it=" + it.toString());
						svcb.saveStudent(it);
						// 保存后，id自动保存到it中
						...
						svcb.saveSyslog(new Syslog(userSession.getId(), userSession.getAlias(), Common.getIP(), 0,
								"增加了学生/学生分组link:" + it.getName() + ",userid=" + it.getId() + ",usergroupid=" + student_group_id,
								"", false));
						// 学生不添加角色对应表
						numberNewStudent++;
			...
			msg = "<br>新添加信息：" + numberNewStudent + "个，<br>已有信息加入当前班级" + numberNewLink + "个<br>重复出现，略过" + numberRepeat
					+ "个" + msg;
			mv.addObject("message", msg);
			mv.addObject("jumpto", Common.getBasePath() + "superadmin/student?sgid=" + student_group_id);
			mv.setViewName("jumperclick");
			return mv;
		}
	}
...
//			throws Exception {
//		System.out.println(this.getClass().getName() + "POST");
//		String pagename = "superadmin/vlink_student2group";
//		UserSession userSession = (UserSession) (session.getAttribute(Common.sessionUser));
//		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
//		String operate = request.getParameter("operate");
//		// --------编辑修改共有的部分---------
...
//		Boolean student_group_is_actived = StringUtils.isBlank(request.getParameter("student_group_is_actived")) ? false : true;
...
//			// 如：用户名必须唯一，手机号必须唯一
//			Map<String, Object> params = new HashMap<String, Object>();
//			 params.put(Vlink_student2group.ID, it.getId());
...
//	}
	...
	public ModelAndView experiment_recordGET(ModelAndView mv, @RequestParam(value = "kw", required = false) String kw,
		...
		System.out.println(this.getClass().getName() + ",operate=" + operate + ",id=" + id + ",pn=" + pn + ",ps=" + ps
				+ ",kw=" + kw + ",clrkw=" + clrkw);
		mv.addObject("navitem", "Experiment");// 菜单项显示当前项
		String pagename = "superadmin/experimentrecord";
		// 新增和编辑，列表页面筛选也需要使用，页面部分都有一些预先读取的内容
		if (StringUtils.isBlank(operate) || operate.equals("add") || operate.equals("edit")) {
			// 新建、编辑、列表的条件筛选
			// 用于制作彩色的标签
			...
			Map<String, Object> curriculumParam = new HashMap<String, Object>() {
				{
					put(Vcurriculum.USER_ID, userSession.getId());
				}
			};
			mv.addObject("curriculumlist", svcb.listVcurriculumByMap(curriculumParam));
			mv.addObject("curriculummap", svcb.mapCurriculum(curriculumParam));
		...
		if (!StringUtils.isBlank(operate)) {
			...
			case "edit": {
				...
				mv.addObject("subtitle", "评阅实验报告");
				mv.addObject("formtitle", "评阅实验报告");
				mv.addObject("pagename", pagename);
				mv.addObject("referer", Common.getRefererUrl());
				...
				if (it == null) {
					...
					String savefolder = request.getServletContext().getRealPath(uploadfolder);
					System.out.println("Admin.vexperiment_recordGET()savefolder=" + savefolder);
					// 读取文件列表
					File file = new File(savefolder);
					Integer filesize = 0;
					List<FileNameHash> filelist = new ArrayList<FileNameHash>();
			...
			System.out.println(
					"Admin.vexperiment_recordGET()student_group_id=" + request.getParameter("student_group_id"));
	...
	public ResponseEntity<byte[]> experimentrecordDownload(HttpServletRequest request,
			...
			@RequestParam("e") Integer experimentid, @RequestParam("u") Integer username, Model model)
			throws Exception {
		...
		System.out.println("Admin.experimentrecordDownload()" + savefolder);
		// 读取文件列表
		String filename = "";
		...
		System.out.println("Admin.download()filename=" + filename);
	...
	public ModelAndView experiment_recordPOST(ModelAndView mv, HttpServletRequest request, HttpSession session)
		...
		case "editform": {
			...
			System.out.println("Admin.experiment_recordPOST()读取下一个");
			...
			if (nextitem.size() > 0) {
				...
				referer = "superadmin/experimentrecord?operate=edit&id=" + nextitemid;
			} else {

				referer = "superadmin/experimentrecord?reviewstate_id=2&curriculum_id=" + item.getCurriculum_id()
	...
	public ModelAndView statexprecordsubmitGET(ModelAndView mv,
		...
		mv.setViewName("superadmin/statexprecordsubmit");

		// 使用shiro控制权限后

		UserSession userSession = (UserSession) (session.getAttribute(Common.sessionUser));// 当前教师

		System.out.println("Admin.statexprecordsubmitGET()curriculumid=" + curriculumid);

		if (userSession.getAuthorizedType().equals("student")) {

		} else {
			// 教师
			System.out.println("Admin.statexprecordsubmitGET()教师统计实验报告结果");
					...
					.listVexperiment_record_miniByMap(new HashMap<String, Object>() {
						{
							...
							// put(Vexperiment_record_mini.EXPERIMENT_IS_ACTIVED, true);
						}
					});

			System.out.println("Admin.statexprecordsubmitGET()exprecords.size=" + exprecords.size());
					...
					.listVlink_experiment_curriculumByMap(new HashMap<String, Object>() {
						{
							...
							put(Vlink_experiment_curriculum.EXPERIMENT_CURRICULUM_IS_ACTIVED, true);
						}
					});
			mv.addObject("curriculums", curriculums);
			System.out.println("Admin.statexprecordsubmitGET()curriculums.size=" + curriculums.size());
			// 读取当前课程的所有学生名单
			...
			System.out.println("Admin.statexprecordsubmitGET()students.size=" + students.size());
	...
	public ModelAndView evprshowGET(ModelAndView mv, HttpServletRequest request, HttpSession session) {
		...
		String pagename = "superadmin/evprreview";
		...
		// TODO: 查看文件
		String param = "&ug=" + evpr.getStudent_group_id() + "&evid=" + evpr.getEvaluation_id() + "&u=" + evpr.getSno()
				+ "&evprid=" + evpr.getId();
		...
		case 7: {
			...
			System.out.println("SuperAdminController.evprshowGET()编程题目");
			// 读取文件列表
			File file = new File(savefolder);
			filesize = 0;
			if (file != null && file.isDirectory()) {
	...
	public ModelAndView evprreviewPOST(ModelAndView mv, HttpServletRequest request, HttpSession session) {
		...
		Integer score = Integer.parseInt(request.getParameter("score"));
		Integer evprid = Integer.parseInt(request.getParameter("evprid"));
		System.out.println("Admin.evprreviewPOST(提交一个小题得分)" + "score=" + score + ",evprid=" + evprid);
	...
	public ResponseEntity<byte[]> evrDownload(HttpServletRequest request) throws Exception {
		...
		System.out.println("Admin.evaluationReviewDownload()");
		...
		String savefolder = request.getServletContext().getRealPath(uploadfolder);
		System.out.println("Admin.evaluationReviewDownload()" + savefolder);
		// 读取文件列表
		String filename = "";
		File targetfile = null;
		File files = new File(savefolder);
	...
	public ModelAndView evaluation_recordPOST(ModelAndView mv, HttpServletRequest request, HttpSession session)
		...
		if (nextlist.size() > 0) {
			// 还有未评阅的
			mv.addObject("jumpto", Common.getBasePath() + "superadmin/evaluationreview/?evid=" + evr.getEvaluation_id()
					+ "&studentid=" + nextlist.get(0).getStudent_id());
		} else {
			// 评阅完成
			mv.addObject("jumpto", Common.getBasePath() + "superadmin/configevaluation?id=" + evr.getEvaluation_id());
		}
	...
	public ModelAndView evpruploadPOST(ModelAndView mv, @PathVariable Integer evprid, HttpServletRequest request,
		...
		System.out.println("Admin.evpruploadPOST()evprid=" + evprid);
		Evaluation_result_record evpr = svcb.getEvaluation_result_recordById(evprid);
		...
		if (file != null) {
			...
			System.out.println("Admin.evpruploadPOST(),file != null");
			File targetfile = new File(savefolder, fileFileName);
			System.out.println("Admin.evpruploadPOST(),targetfile=" + targetfile.getAbsolutePath());
		...
		mv.addObject("jumpto", Common.getBasePath() + "superadmin/evprreview?evprid=" + evprid);
		mv.setViewName("jumper");
	...
	public ModelAndView evsubmitGET(ModelAndView mv, @PathVariable Integer evid, HttpServletRequest request,
		...
		mv.addObject("jumpto", Common.getBasePath() + "superadmin/configevaluation?id=" + evid);
		mv.setViewName("jumper");
		return mv;
	}
}
