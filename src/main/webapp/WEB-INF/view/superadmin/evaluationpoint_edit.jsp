							...
							<div class="col-md-12">
								...
								<c:choose>
									<c:when test="${item.type==1}">
										<%-- 填空题修改 --%>
										<form class="form-horizontal padding-horizontal-sm" method="post"
											action="${curl}">
											<%-- 指定操作类型 --%>
											<input type="hidden" name="operate" value="editform" />
											<%-- 记录来源页面 --%>
											...
											<%-- 填空题默认1个标准答案，可以调整为多个 --%>
											<input type="hidden" id="itnum1" name="itnum1"
												...
												<div class="col-sm-10">
													<select class="selectbox" id="subject_id" name="subject_id"
														...
														<c:forEach var="it" items="${subjectlist}" varStatus="i">
															<option value="${it.id}"
																<c:if test="${it.id==item.subject_id}">selected</c:if>>${it.name}</option>
														</c:forEach>
													</select>
												</div>
											</div>
											...
											<div id="form1">
												...
												<c:forEach var="ita" items="${item.answers}" varStatus="i">
													<div class="form-group">
														<label for="evpContent" class="col-sm-2 control-label">正确答案${i.count}</label>
														<div class="col-sm-10">
															<input type="text" class="form-control"
																id="evpa1${i.count}" name="evpa1${i.count}"
																value="${ita.answer}" placeholder="正确答案" required>
														</div>
													</div>
												</c:forEach>
											</div>
											<div class="form-group">
												<div class="col-sm-offset-2 col-sm-10">
													<input type="submit" class="btn btn-success" value="修改题目信息" />
												</div>
											</div>
										</form>
										<%-- 填空题 --%>
													...
													<div class="form-group">
														...
														<div class="col-sm-8">
															<input class="form-control" type="text"
																id="evpa3${i.count}" name="evpa3${i.count}"
																value="${ita.answer}" placeholder="备选答案" required />
														</div>
														<div class="col-sm-2">
															<div
																...
																<input type="checkbox" id="tevpa3${i.count}"
																	name="tevpa3${i.count}"
																	<c:if test="${ita.correct}">checked</c:if>> <label
													...
													<div class="col-sm-2">
														<div
															...
															<input type="checkbox" id="tevpa51" name="tevpa51"
																<c:if test="${item.answers[0].correct}">checked</c:if> />
													...
													<div class="col-sm-2">
														<div
															...
															<input type="checkbox" id="tevpa52" name="tevpa52"
																<c:if test="${item.answers[1].correct}">checked</c:if> />
												...
												<div class="form-group">
													...
													<div class="col-sm-10">
														<textarea class="form-control" id="evptitle1"
															name="evptitle1"
															style="width: 100%; height: 300px; visibility: hidden;">${item.title}</textarea>
													</div>
												</div>
											</div>
											<div class="form-group margin-bottom-xl">
												<div class="col-sm-9 col-sm-push-2">
													<input type="submit" class="btn btn-primary" value="修改题目信息" />
												</div>
											</div>
										</form>
									</c:when>
									...
									<c:when test="${item.type==7}">
										...
										<form class="form-horizontal padding-horizontal-sm " method="post"
											...
											<%-- 默认答案数，就算没有答案也要有这个参数 --%>
											<input type="hidden" id="itnum7" name="itnum7" value="2" />
											<%-- 当前题号 --%>
											<input type="hidden" name="id" value="${param.id}" />
											<%-- 题目所属科目 --%>
											<!-- selectbox begin -->
											...
											<div id="form7">
												...
												<div class="form-group">
													<label for="title"
														class="col-sm-2 control-label text-right">标题</label>
													<div class="col-sm-10">
														<input type="text" class="form-control" id="title"
															name="title" placeholder="标题"  value="${item.title}"  required>
													</div>
												</div>
												<div class="clearfix"></div>
												<div class="form-group">
													<label for="evptitle1"
														class="col-sm-2 control-label text-right">题目描述 </label>
													<div class="col-sm-10">
														<textarea class="form-control" id="evptitle1"
															name="evptitle1"
															style="width: 100%; height: 300px; visibility: hidden;">${item.evpcontent}</textarea>
													</div>
												</div>
												<div class="clearfix"></div>
												<div class="form-group">
													<label for="evpin"
														class="col-sm-2 control-label text-right">输入</label>
													<div class="col-sm-10">
														<input type="text" class="form-control" id="evpin"
															name="evpin" placeholder="输入"    value="${item.evpin}"  required>
													</div>
												</div>
												<div class="clearfix"></div>
												<div class="form-group">
													<label for="evpout"
														class="col-sm-2 control-label text-right">输出</label>
													<div class="col-sm-10">
														<input type="text" class="form-control" id="evpout"
															name="evpout" placeholder="输出"    value="${item.evpout}"  required>
													</div>
												</div>
												<div class="clearfix"></div>
												<div class="form-group">
													<label for="evpexamplein"
														class="col-sm-2 control-label text-right">样例输入</label>
													<div class="col-sm-10">
														<input type="text" class="form-control" id="evpexamplein"
															name="evpexamplein" placeholder="样例输入"    value="${item.evpexamplein}"  required>
													</div>
												</div>
												<div class="clearfix"></div>
												<div class="form-group">
													<label for="evpexampleout"
														class="col-sm-2 control-label text-right">样例输出</label>
													<div class="col-sm-10">
														<input type="text" class="form-control" id="evpexampleout"
															name="evpexampleout" placeholder="样例输出"    value="${item.evpexampleout}"  required>
													</div>
												</div>
												<div class="clearfix"></div>
												<div class="form-group">
													<label for="datalimit"
														class="col-sm-2 control-label text-right">数据范围限制</label>
													<div class="col-sm-10">
														<input type="text" class="form-control" id="datalimit"
															name="datalimit" placeholder="数据范围限制"   value="${item.datalimit}" >
													</div>
												</div>
