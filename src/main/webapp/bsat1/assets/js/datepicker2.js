...
function onLoadTimeChoiceDemo() {
	var demo = '<div class="borain-timeChoice-body">'
			...
			+ '<span></span>'
			+ '<a class="minus" href="javascript:void (0);"><i class="fa fa-angle-down" aria-hidden="true"></i></a>'
			+ '</div>' + '</div>' + '<div class="operateBox clearFix">'
			+ '<a class="cancel fl" href="javascript:void (0);">取消</a>'
			+ '<a class="submit fr" href="javascript:void (0);">确认</a>'
			+ '</div>' + '</div>' + '</div>' + '</div>';
	$("body").append(demo);
...
function borainTimeChoiceShow(obj) {
	...
	var objWidth = obj.width() + parseInt(obj.css("border-left"))
			+ parseInt(obj.css("border-right"))
			+ parseInt(obj.css("padding-left"))
			+ parseInt(obj.css("padding-right"));
	var objHeight = obj.height() + parseInt(obj.css("border-top"))
			+ parseInt(obj.css("border-bottom"))
...
function hasEndEndTimeDateEvent(start, end, level, less) {
	...
	if (start.val() == "" || start.val() == null) {
		...
		if (end.val() == "" || end.val() == null) {
			...
			var lastDay = box.find(".choiceDayBox").find(".day span").not(
					".ban").length;
			if (level == "YM") {
				if (month < 12) {
					month = month + 1;
				...
				endVal = year + "-" + month + "-" + day + " " + hour + ":"
						+ minute;
			}
			end.val(endVal);
			//console.log("end.offset().left="+end.offset().left);
			if (typeof(end.offset()) != "undefined"){
				notEndOldTimeDateEvent(end, level, less);
			}
		} else {
			endDate = end.val();
...
function onLoadChoiceMonth(less) {
	var box = $(".borain-timeChoice");
	var choiceMinBox = box.find(".choiceBox").find(".choiceMinBox").find(
			".month");
	var obj;
	var year = parseInt(box.find(".choiceBox .yearBox .showBox").find(".year")
			.text());
	choiceMinBox.html("");
	var nowYear = new Date().getFullYear();
	var nowMonth = new Date().getMonth() + 1;
	var choiceMonth = parseInt(box.find(".choiceBox .yearBox .showBox").find(
			".month").text());
	var i = 1;
	// 如果不可小于当前时间
...
function choiceYearBoxAppend(year, b, c, yearHead, yearBody, less) {
	var box = $(".borain-timeChoice");
	var choiceYearBox = box.find(".choiceBox").find(".choiceYearBox").find(
			".year");
	var obj;
	var yearTitle = box.find(".choiceBox").find(".choiceYearBox").find(
			".yearTitle");
	var nowYear = new Date().getFullYear();
	choiceYearBox.html("");
	for (var a = b; a <= c; a++) {
	...
	yearTitle.find("span").html(
			"<em>" + choiceYearBox.find("span").eq(0).text() + "</em>" + "-"
					+ "<em>" + choiceYearBox.find("span").eq(19).text()
					+ "</em>");
...
function choiceYearSpanClick(year, less) {
	...
	// 给按钮样式
	// 打开对应的选择范围
	box.find(".yearBox .showBox").find(".year").addClass("active").siblings()
			.removeClass("active");
	var choiceBox = box.find(".choiceYearBox");
	box.find(".beChoice").css("display", "none");
	choiceBox.css("display", "block");
	// 执行点击事件
	choiceBox.find(".year").find("span").not(".not").unbind("click").click(
			function() {
				var year = $(this).text();
				...
				choiceMonthSpanClick(year, less);
				box.find(".yearBox .showBox").find(".month").addClass("active")
...
function choiceMonthSpanClick(year, less) {
	...
	// box.find(".yearBox
	// .showBox").find(".month").addClass("active").siblings().removeClass("active");
	// box.find(".beChoice").css("display","none");
	// choiceBox.css("display","block");
	onLoadChoiceMonth(less);
	choiceBox.find(".month").find("span").not(".not").unbind("click").click(
			function() {
				...
				if (box.hasClass("YM")) {
					...
					// 非年月选择到达月选择后都要进行天选择
					getDaysInMonth(year, month, less);
					box.find(".choiceDayBox").css("display", "block").siblings(
							".beChoice").css("display", "none");
					box.find(".yearBox .showBox").find(".month").removeClass(
							"active");
					var choiceDay = parseInt(box.find(
							".revealBox .nowBox .nowDay").find("em").text());
					var lastDay = box.find(".choiceBox .choiceDayBox .day")
							.find("span").not(".ban").length;
...
function choiceYearChange(less, year) {
	...
	var activeYear;
	var obj;
	upBtn.unbind("click").click(
			function() {
				var firstYear = parseInt(yearBox.find("span").eq(0).text());
				activeYear = box.find(".choiceBox .yearBox").find(
						".showBox .year").text();
				yearBox.html("");
				for (var i = 0; i < 20; i++) {
					firstYear--;
				...
				yearTitle.find("span").html(
						"<em>" + choiceYearBox.find("span").eq(0).text()
								+ "</em>" + "-" + "<em>"
								+ choiceYearBox.find("span").eq(19).text()
								+ "</em>");
				choiceYearLess(less, year);
				choiceYearSpanClick();
			});
	downBtn.unbind("click").click(
			function() {
...
function yearMonthClickChange(less) {
	...
	var month;
	// 点击年月前进后退按钮
	box.find(".choiceBox .yearBox").find(".back").unbind("click").click(
			function() {
				backClick(less);
				var a = parseInt(box.find(".choiceBox .showBox").find(".month")
						.text());
				box.find(".choiceMinBox .month").find("span").each(
						function() {
	...
	box.find(".choiceBox .yearBox").find(".forward").unbind("click").click(
			function() {
				forwardClick(less);
				var a = parseInt(box.find(".choiceBox .showBox").find(".month")
						.text());
		...
		if (choiceYear == nowYear) {
			if (choiceMonth > nowMonth) {
				if (choiceMonth > 1) {
					...
					if (box.hasClass("YM")) {
						...
						getDaysInMonth(year, month, less);
						choiceDay = parseInt(box.find(
								".revealBox .nowBox .nowDay").find("em").text());
						lastDay = box.find(".choiceBox .choiceDayBox .day")
								.find("span").not(".ban").length;
						if (choiceDay > lastDay) {
							choiceDay = lastDay;
						}
			...
			if (choiceMonth > 1) {
				...
				if (box.hasClass("YM")) {
					...
					getDaysInMonth(year, month, less);
					choiceDay = parseInt(box.find(".revealBox .nowBox .nowDay")
							.find("em").text());
					lastDay = box.find(".choiceBox .choiceDayBox .day").find(
							"span").not(".ban").length;
					if (choiceDay > lastDay) {
						choiceDay = lastDay;
					}
...
function forwardClick(less) {
	...
	if (monthBox.text() < 12) {
		...
		if (box.hasClass("YM")) {
			...
			getDaysInMonth(year, month, less);
			choiceDay = parseInt(box.find(".revealBox .nowBox .nowDay").find(
					"em").text());
			lastDay = box.find(".choiceBox .choiceDayBox .day").find("span")
					.not(".ban").length;
			if (choiceDay > lastDay) {
				choiceDay = lastDay;
			}
...
function getDateVal(strDate) {
	...
	if (typeof (strDate) == "undefined") {
		// console.log("strDate="+strDate+"空，给个值，免得出错");
		strDate = "2009-08-22 00:00";
	}
	// console.log("strDate="+strDate+"开始转换");
...
function todayStyle(year, month, less) {
	...
	// var choiceYear = parseInt(box.find(".choiceBox .yearBox
	// .showBox").find(".year").text());
	var choiceMonth = parseInt(month);
	// var choiceMonth = parseInt(box.find(".choiceBox .yearBox
	// .showBox").find(".month").text());
	if (choiceYear == nowYear && choiceMonth == nowMonth) {
		box.find(".choiceBox").find(".choiceDayBox").find(".day").find("span")
				.not(".ban").each(function() {
	...
	if (less == true || less == "true") {
		if (choiceYear == nowYear) {
			if (choiceMonth == nowMonth) {
				box.find(".choiceBox").find(".choiceDayBox").find(".day").find(
						"span").not(".ban").each(function() {
					if (parseInt($(this).text()) < today) {
						$(this).addClass("not");
					}
				});
				...
				box.find(".choiceBox").find(".choiceDayBox").find(".day").find(
						"span").not(".ban").addClass("not");
			} else {
				box.find(".choiceBox").find(".choiceDayBox").find(".day").find(
						"span").not(".ban").removeClass("not");
			}
		} else if (choiceYear < nowYear) {
			box.find(".choiceBox").find(".choiceDayBox").find(".day").find(
					"span").not(".ban").addClass("not");
...
function timeChoiceValue(year, month, day, hour, minute) {
	...
	box.find(".choiceBox").find(".choiceYearBox").find(".year").find("span")
			.not(".ban").each(
					function() {
	...
	box.find(".choiceBox").find(".choiceMinBox ").find(".month").find("span")
			.not(".ban").each(
					function() {
	...
	box.find(".choiceBox").find(".choiceDayBox").find(".day").find("span").not(
			".ban").each(function() {
			...
			function() {
				if ($(this).hasClass("ban")) {
					...
					if ($(this).hasClass("not")) {
						...
						$(this).siblings().removeClass("active");
						$(this).addClass("active");
						var year = choiceBox.find(".yearBox .showBox").find(
								".year").text();
						var month = choiceBox.find(".yearBox .showBox").find(
								".month").text();
						var day = $(this).text();
...
function setHour() {
	...
	hourAdd.unbind("click").click(
			function() {
				var hour = parseInt(box.find(".revealBox .timeBox").find(
						".hourBox span").text());
				if (hour < 23) {
					hour = hour + 1;
				} else {
				...
				box.find(".revealBox .timeBox").find(".hourBox span")
						.text(hour);
			});
	hourMinus.unbind("click").click(
			function() {
				var hour = parseInt(box.find(".revealBox .timeBox").find(
						".hourBox span").text());
				if (hour > 0) {
...
function setMinute() {
	...
	minAdd.unbind("click").click(
			function() {
				var min = parseInt(box.find(".revealBox .timeBox").find(
						".minBox span").text());
				if (min < 59) {
					min = min + 1;
				} else {
...
function notEndTimeChoiceSubmit(obj) {
	...
	submitBtn.unbind("click").click(
			function() {
				year = box.find(".revealBox .nowBox").find(".nowYear em")
						.text();
				month = box.find(".revealBox .nowBox").find(".nowMonth em")
						.text();
				// 根据级别来取值
				if (box.hasClass("YM")) {
					val = year + "-" + month;
					...
					day = box.find(".revealBox .nowBox").find(".nowDay em")
							.text();
					val = year + "-" + month + "-" + day;
					obj.val(val);
				} else {
					day = box.find(".revealBox .nowBox").find(".nowDay em")
							.text();
					hour = box.find(".revealBox .timeBox")
							.find(".hourBox span").text();
					minute = box.find(".revealBox .timeBox").find(
							".minBox span").text();
...
function hasEndTimeChoiceSubmit(obj, start, end, level, less) {
			...
			function() {
				...
				if (obj.is(start)) {
					if (end.val() == null || end.val() == "") {
						...
						console.log(endDate);
						endDate = getDateVal(endDate);
						year = box.find(".revealBox .nowBox").find(
								".nowYear em").text();
						month = box.find(".revealBox .nowBox").find(
								".nowMonth em").text();
						// 根据级别来取值
						if (box.hasClass("YM")) {
							choiceStartVal = year + "-" + month + "-1 0:0";
						} else if (box.hasClass("YMD")) {
							day = box.find(".revealBox .nowBox").find(
									".nowDay em").text();
							choiceStartVal = year + "-" + month + "-" + day
									+ " 0:0";
						} else {
							day = box.find(".revealBox .nowBox").find(
									".nowDay em").text();
							hour = box.find(".revealBox .timeBox").find(
									".hourBox span").text();
							minute = box.find(".revealBox .timeBox").find(
									".minBox span").text();
							choiceStartVal = year + "-" + month + "-" + day
	...
	function sub() {
		...
		if (box.hasClass("YM")) {
			...
			minute = box.find(".revealBox .timeBox").find(".minBox span")
					.text();
